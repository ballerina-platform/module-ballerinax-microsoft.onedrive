...
public client class Client {
    ...
    # + return - An array of type `DriveItem` if sucess. Else `Error`.
    remote isolated function getItemsSharedWithMe(@display {label: "Optional query parameters"} string[] queryParams = []) 
    ...
    remote isolated function createFolderById(@display {label: "Max number of results"} string parentFolderId, 
                                              @display {label: "Max number of results"} FolderMetadata folderMetadata) 
                                              returns @tainted DriveItem|Error { 
    ...
    remote isolated function uploadDriveItemToFolderById(string parentFolderId, string fileName, 
                                                         stream<byte[],io:Error?> binaryStream, string mediaType) 
                                                         returns @tainted DriveItem|Error {
    ...
    remote isolated function uploadDriveItemToFolderByIdTest(string parentFolderId, string fileName, byte[] byteArray, 
    ...
    remote isolated function uploadDriveItemToFolderByPathTest(string parentFolderPath, string fileName, byte[] byteArray, 
                                                               string mediaType) returns @tainted DriveItem|Error {
        string path = check createPathBasedUrl([LOGGED_IN_USER, DRIVE_RESOURCE, DRIVE_ROOT], 
            string `${parentFolderPath}/${fileName}`, [CONTENT_OF_DRIVE_ITEM]);
        return uploadDriveItemByteArray(self.httpClient, <@untainted>path, byteArray, mediaType); 
    ...
    remote isolated function replaceFileUsingIdTest(string itemId, byte[] byteArray, string mediaType) returns 
                                                    @tainted DriveItem|Error {
        string path = check createUrl([LOGGED_IN_USER, DRIVE_RESOURCE, ALL_DRIVE_ITEMS, itemId, 
            CONTENT_OF_DRIVE_ITEM]);
        return uploadDriveItemByteArray(self.httpClient, <@untainted>path, byteArray, mediaType); 
    ...
    remote isolated function replaceFileUsingPathTest(string itemPath, byte[] byteArray, string mediaType) returns 
                                                      @tainted DriveItem|Error {
        string path = check createPathBasedUrl([LOGGED_IN_USER, DRIVE_RESOURCE, DRIVE_ROOT], itemPath, 
            [CONTENT_OF_DRIVE_ITEM]);
        return uploadDriveItemByteArray(self.httpClient, <@untainted>path, byteArray, mediaType); 
    ...
    # + mediaType - The media type of the uploading file
    # + return - A record of type `DriveItem` if sucess. Else `Error`.
    remote isolated function replaceFileUsingId(string itemId, stream<byte[],io:Error?> binaryStream, string mediaType) 
                                                returns @tainted DriveItem|Error {
        string path = check createUrl([LOGGED_IN_USER, DRIVE_RESOURCE, ALL_DRIVE_ITEMS, itemId, 
            CONTENT_OF_DRIVE_ITEM]);
        return uploadDriveItem(self.httpClient, <@untainted>path, binaryStream, mediaType); 
    ...
    # + mediaType - The media type of the uploading file
    # + return - A record of type `DriveItem` if sucess. Else `Error`.
    remote isolated function replaceFileUsingPath(string itemPath, stream<byte[],io:Error?> binaryStream, 
                                                  string mediaType) returns @tainted DriveItem|Error {
        string path = check createPathBasedUrl([LOGGED_IN_USER, DRIVE_RESOURCE, DRIVE_ROOT], itemPath, 
            [CONTENT_OF_DRIVE_ITEM]);
        return uploadDriveItem(self.httpClient, <@untainted>path, binaryStream, mediaType); 
    ...
    # + return - A record of type `DriveItem` if sucess. Else `Error`.
    remote function resumableUploadDriveItem(string itemPath, ItemInfo itemInfo, stream<io:Block,io:Error?> binaryStream) 
                                             returns @tainted DriveItem|Error { 
